...
async function main() {
    console.log("🚀 Starting Host Fair Verification Flow (TS)...")
    ...
    // @ts-ignore - bypassing strict type checks for quick script
    ...
    if (universities.length === 0) {
        console.warn("⚠️ No universities found, creating dummies...")
        // Create dummy if needed
        const user = await prisma.user.create({
            data: { email: `dummy-${Date.now()}@test.com`, role: 'UNIVERSITY' }
        })
        await prisma.university.create({
            data: { userId: user.id, institutionName: "Dummy Uni", country: "USA", contactEmail: user.email }
        })
        // Refetch
    }
    const targetUnis = await prisma.university.findMany({ take: 2 })
