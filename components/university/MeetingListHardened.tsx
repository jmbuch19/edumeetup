...
import { Calendar, CheckCircle, Clock, MapPin, Video, XCircle } from 'lucide-react'
import { confirmUniversityMeeting, cancelUniversityMeeting } from '@/app/actions/meeting-university'
...
export default function MeetingListHardened({ meetings }: { meetings: Meeting[] }) {
    ...
    if (!meetings.length) {
        return <div className="text-center py-12 text-gray-500">No meetings found.</div>
    }
                    ...
                    <Card key={meeting.id}>
                        <CardHeader>
                            <div className="flex items-start justify-between gap-4">
                                <div>
                                    <CardTitle className="text-lg">{meeting.student.fullName}</CardTitle>
                        ...
                        <CardContent className="space-y-3 text-sm">
                            ...
                            {meeting.studentQuestions && <div><strong>Questions:</strong> {meeting.studentQuestions}</div>}
                            {meeting.student.cvFileName && meeting.student.id && (
                                <a className="text-blue-600 hover:underline" href={`/api/cv/${meeting.student.id}`} target="_blank" rel="noreferrer">View student CV</a>
                            )}
                            {meeting.status === 'CONFIRMED' && (meeting.hostRoomUrl || meeting.meetingLink) && (
                                <a className="inline-flex items-center gap-1 text-green-700 hover:underline" href={meeting.hostRoomUrl || meeting.meetingLink || '#'} target="_blank" rel="noreferrer">
                        ...
                        <CardFooter className="flex flex-wrap justify-end gap-2">
                            {meeting.status === 'PENDING' && (
                                <>
                                    <Button
                                        ...
                                        onClick={() => run(meeting.id, () => confirmUniversityMeeting(meeting.id))}
                                    >
                                    ...
                                    <Button
                                        ...
                                        variant="destructive"
                                        disabled={processing === meeting.id}
                                        onClick={() => run(meeting.id, () => cancelUniversityMeeting(meeting.id, 'Declined by university'))}
                                    >
                            ...
                            {meeting.status === 'CONFIRMED' && (
                                <>
                                    ...
                                    <Button
                                        ...
                                        variant="destructive"
                                        disabled={processing === meeting.id}
                                        onClick={() => run(meeting.id, () => cancelUniversityMeeting(meeting.id))}
                                    >
                                        Cancel Meeting
                                    </Button>
                                </>
                            ...
                            {meeting.status === 'RESCHEDULE_PROPOSED' && meeting.rescheduleProposedBy !== 'STUDENT' && (
                                <span className="text-sm text-gray-500">Waiting for the student to respond.</span>
                            )}
                        </CardFooter>
                    </Card>
                )
