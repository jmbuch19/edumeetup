...
export async function submitProctorRequest(
    ...
    if (new Date(examEndDate) < new Date(examStartDate)) {
        return { error: 'Exam end date must be on or after the start date.' }
    }
  ...
  `
    ...
    // ── University confirmation email ─────────────────────────────────────────
    const uniEmail = university.repEmail || university.contactEmail
    if (uniEmail) {
        const uniHtml = `
      <p>Dear ${university.repName || 'Team'},</p>
      <p>Your proctor service request has been received and is pending review by our team.</p>
      <div class="info-box">
        <div class="info-row"><span class="info-label">Exam Period:</span> ${fmt(examStartDate)} – ${fmt(examEndDate)}</div>
        <div class="info-row"><span class="info-label">Subjects:</span> ${subjects}</div>
        <div class="info-row"><span class="info-label">Students:</span> ${studentCount}</div>
        <div class="info-row"><span class="info-label">Duration:</span> ${durationMinutes} min</div>
    ...
    await prisma.universityNotification.create({
        data: {
            ...
            message: `Your proctor request for ${studentCount} students (${fmt(examStartDate)} – ${fmt(examEndDate)}) has been received.`,
        },
    })
    ...
    await logSystemEvent({
        level: 'INFO',
        type: 'SYSTEM_EVENT',
        message: `Proctor request submitted: ${university.institutionName} · ${studentCount} students`,
